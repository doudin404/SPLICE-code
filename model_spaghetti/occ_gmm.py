...
from model_spaghetti import models_utils, transformer
...
def get_p_direct(splitted: Union[Tuple[torch.Tensor, ...], List[torch.Tensor]]) -> torch.Tensor:
    raw_base = []
...
def split_gm(splitted: Union[Tuple[torch.Tensor, ...], List[torch.Tensor]]) -> Union[Tuple[torch.Tensor, ...], List[torch.Tensor]]:
...
class OccupancyMlP(nn.Module):
    ...
    def __init__(self, opt: Options):
        ...
        for i in range(0, len(dims) - 1):
            layers.append(nn.utils.parametrizations.weight_norm(nn.Linear(dims[i], dims[i + 1])))
        self.layers = nn.ModuleList(layers)


class OccupancyNetwork(nn.Module):

    def get_pos(self, coords: torch.Tensor):
        pos = self.pos_encoder(coords)
        pos = torch.cat((coords, pos), dim=2)
        return pos

    def forward_attention(self, coords: torch.Tensor, zh: torch.Tensor, mask: Optional[torch.Tensor] = None, alpha: Optional[torch.Tensor] = None) -> Union[Tuple[torch.Tensor, ...], List[torch.Tensor]]:
        pos = self.get_pos(coords)
        _, attn = self.occ_transformer.forward_with_attention(pos, zh, mask, alpha)
        return attn

    def forward(self, coords: torch.Tensor, zh: torch.Tensor, mask: Optional[torch.Tensor] = None, alpha: Optional[torch.Tensor] = None) -> torch.Tensor:
        pos = self.get_pos(coords)
...
class DecompositionControl(models_utils.Model):
    ...
    def forward_split(self, x: torch.Tensor) -> Tuple[torch.Tensor, Union[Tuple[torch.Tensor, ...], List[torch.Tensor]]]:
        b = x.shape[0]
    ...
    @staticmethod
    def apply_gmm_affine(gmms: Union[Tuple[torch.Tensor, ...], List[torch.Tensor]], affine: torch.Tensor):
        mu, p, phi, eigen = gmms
        if affine.dim() == 2:
    ...
    @staticmethod
    def concat_gmm(gmm_a: Union[Tuple[torch.Tensor, ...], List[torch.Tensor]], gmm_b: Union[Tuple[torch.Tensor, ...], List[torch.Tensor]]):
        out = []
    ...
    def forward_mid(self, zs) -> Tuple[torch.Tensor, Union[Tuple[torch.Tensor, ...], List[torch.Tensor]]]:
    ...
    def forward(self, z_init) -> Tuple[torch.Tensor, Union[Tuple[torch.Tensor, ...], List[torch.Tensor]]]:
...
class Spaghetti(models_utils.Model):
    ...
    def get_z(self, item: torch.Tensor):
        return self.z(item)
    ...
    def interpolate_higher(self, z: torch.Tensor, num_between: Optional[int] = None):
    ...
    def get_disentanglement(self, items: torch.Tensor):
    ...
    def get_embeddings(self, item: torch.Tensor):
        z = self.get_z(item)
    ...
    def merge_zh(self, zh, gmms, mask: Optional[torch.Tensor] = None) -> Union[Tuple[Optional[torch.Tensor], ...], List[Optional[torch.Tensor]]]:
        zh_ = self.merge_zh_step_a(zh, gmms)
        zh_, attn = self.mixing_network.forward_with_attention(zh_, mask=mask)
        return zh_, attn

    def forward_b(self, x, zh, gmms, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        zh, _ = self.merge_zh(zh, gmms, mask)
        return self.occupancy_network(x, zh, mask)

    def forward_a(self, item: torch.Tensor):
        zh, z, gmms = self.get_embeddings(item)
        return zh, z, gmms

    def get_attention(self, x, item) -> Union[Tuple[torch.Tensor, ...], List[torch.Tensor]]:
        zh, z, gmms = self.forward_a(item)
        zh, _ = self.merge_zh(zh, gmms)
        return self.occupancy_network.forward_attention(x, zh)

    def forward(self, x, item: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, Union[Tuple[torch.Tensor, ...], List[torch.Tensor]], torch.Tensor]:
        zh, z, gmms = self.forward_a(item)
        return self.forward_b(x, zh, gmms), z, gmms, zh

    def forward_mid(self, x: torch.Tensor, zh: torch.Tensor) -> Tuple[torch.Tensor, Union[Tuple[torch.Tensor, ...], List[torch.Tensor]]]:
