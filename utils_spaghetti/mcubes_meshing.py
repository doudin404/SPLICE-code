...
import constants
from utils_spaghetti.train_utils import Logger


def mcubes_skimage(pytorch_3d_occ_tensor: torch.Tensor, voxel_grid_origin: List[float], voxel_size: float) -> Tuple[torch.Tensor, torch.Tensor]:
...
class MarchingCubesMeshing:
    ...
    def fill_samples(self, decoder, samples, device: Optional[torch.device] = None) -> torch.Tensor:
    ...
    def fill_recursive(self, decoder, samples: torch.Tensor, stride: int, base_res: int, depth: int) -> torch.Tensor:
    ...
    @staticmethod
    def get_res_samples(res: int) -> torch.Tensor:
    ...
    def __init__(self, device: torch.device, max_batch: int = 64 ** 3, min_res: int = 64, scale: float = 1, verbose: bool = False):
        self.device = device
...
def create_mesh_old(decoder, res=256, max_batch=64 ** 3, scale=1, device=torch.device('cpu'), verbose=False, get_time: bool = False):
