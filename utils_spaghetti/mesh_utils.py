...
def scale_all(*values: torch.Tensor):
...
def get_faces_normals(mesh: Union[Tuple[torch.Tensor, Optional[torch.Tensor]], torch.Tensor]) -> torch.Tensor:
...
def compute_face_areas(mesh: Union[Tuple[torch.Tensor, Optional[torch.Tensor]], torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
...
def check_sign_area(*meshes: Tuple[torch.Tensor, Optional[torch.Tensor]]) -> bool:
    for mesh in meshes:
        face_normals = get_faces_normals(mesh)
...
def to_numpy(*tensors: torch.Tensor) -> Union[List[np.ndarray], Tuple[np.ndarray, ...]]:
    params = [param.detach().cpu().numpy() if type(param) is torch.Tensor else param for param in tensors]
    return params


def create_mapper(mask: torch.Tensor) -> torch.Tensor:
...
def mesh_center(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]]):
    return mesh[0].mean(0)


def get_center(vs: torch.Tensor) -> torch.Tensor:
    max_vals = vs.max(0)[0]
    min_vals = vs.min(0)[0]
...
def to_center(vs: torch.Tensor):
    vs -= get_center(vs)[None, :]
    return vs


def scale_by_ref(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], ref_mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], in_place=True, scale=1.):
    vs, _ = ref_mesh
    if not in_place:
        vs = vs.clone()
...
def to_unit_sphere(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], in_place: bool = True, scale=1.) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
    vs, faces = mesh
    if not in_place:
        vs = vs.clone()
    vs = to_center(vs)
...
def scale_from_ref(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], center: torch.Tensor, scale: float, in_place: bool = True) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
    vs, faces = mesh
    if not in_place:
        vs = vs.clone()
    vs -= center[None, :]
...
def to_unit_cube(*meshes: Union[Tuple[torch.Tensor, Optional[torch.Tensor]], torch.Tensor], scale=1, in_place: bool = True) -> Tuple[Union[Union[Tuple[torch.Tensor, Optional[torch.Tensor]], torch.Tensor], Tuple[Union[Tuple[torch.Tensor, Optional[torch.Tensor]], torch.Tensor], ...]], Tuple[torch.Tensor, float]]:
    remove_me = 0
    meshes = [(mesh, remove_me) if type(mesh) is torch.Tensor else mesh for mesh in meshes]
    vs, faces = meshes[0]
    max_vals = vs.max(0)[0]
    min_vals = vs.min(0)[0]
...
def get_edges_ind(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]]) -> torch.Tensor:
    vs, faces = mesh
...
def edge_lengths(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], edges_ind: Optional[torch.Tensor] = None) -> torch.Tensor:
    vs, faces = mesh
...
# in place
def to_unit_edge(*meshes: Tuple[torch.Tensor, Optional[torch.Tensor]]) -> Tuple[Union[Tuple[torch.Tensor, Optional[torch.Tensor]], Tuple[Tuple[torch.Tensor, Optional[torch.Tensor]], ...]], Tuple[torch.Tensor, float]]:
    ref = meshes[0]
    center = ref[0].mean(0)
    ratio = edge_lengths(ref).mean().item()
    for mesh in meshes:
...
def to(tensors, device: torch.device) -> Union[Tuple[torch.Tensor, Optional[torch.Tensor]], Union[List[torch.Tensor], Tuple[torch.Tensor, ...]], torch.Tensor]:
    out = []
...
def clone(*tensors: Union[torch.Tensor, Union[List[torch.Tensor], Tuple[torch.Tensor, ...]]]) -> Union[Union[List[torch.Tensor], Tuple[torch.Tensor, ...]], Tuple[torch.Tensor, Optional[torch.Tensor]]]:
    out = []
    for t in tensors:
...
def get_box(w: float, h: float, d: float) -> Tuple[torch.Tensor, torch.Tensor]:
...
def interpolate_vs(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], faces_inds: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
    vs = mesh[0][mesh[1][faces_inds]]
    vs = vs * weights[:, :, None]
    return vs.sum(1)


def sample_uvw(shape, device: torch.device):
...
def get_sampled_fe(fe: torch.Tensor, mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], face_ids: torch.Tensor, uvw: Optional[torch.Tensor]) -> torch.Tensor:
    # to_squeeze =
    if fe.dim() == 1:
        fe = fe.unsqueeze(1)
    if uvw is None:
...
def sample_on_faces(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], num_samples: int) -> Tuple[torch.Tensor, torch.Tensor]:
    vs, faces = mesh
...
class SampleBy(enum.Enum):
    AREAS = 0
    FACES = 1
    HYB = 2


def sample_on_mesh(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], num_samples: int, face_areas: Optional[torch.Tensor] = None,
                   sample_s: SampleBy = SampleBy.HYB) -> Union[List[Optional[torch.Tensor]], Tuple[Optional[torch.Tensor], ...]]:
    vs, faces = mesh
...
def get_samples(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], num_samples: int, sample_s: SampleBy, *features: torch.Tensor) -> Tuple[Union[torch.Tensor, List[torch.Tensor]], torch.Tensor, Optional[torch.Tensor]]:
...
def find_barycentric(vs: torch.Tensor, triangles: torch.Tensor) -> torch.Tensor:
...
def from_barycentric(mesh: Union[Tuple[torch.Tensor, Optional[torch.Tensor]], torch.Tensor], face_ids: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
    if type(mesh) is not torch.Tensor:
        triangles: torch.Tensor = mesh[0][mesh[1]]
    else:
        triangles: torch.Tensor = mesh
...
def check_circle_angles(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], center_ind: int, select: torch.Tensor) -> bool:
    vs, _ = mesh
...
def vs_over_triangle(vs_mid: torch.Tensor, triangle: torch.Tensor, normals=None) -> torch.Tensor:
    if vs_mid.dim() == 1:
...
def get_dist_mat(a: torch.Tensor, b: torch.Tensor, batch_size: int = 1000, sqrt: bool = False) -> torch.Tensor:
    """
       :param a:
       :param b:
    ...
    all_dist: torch.Tensor = torch.cat(dist_list, dim=0)
    if sqrt:
        all_dist = all_dist.sqrt_()
    return all_dist


def naive_knn(k: int, dist_mat: torch.Tensor, is_biknn=True):
    """
    :param k:
    :param dist_mat:
...
def simple_chamfer(a: torch.Tensor, b: torch.Tensor, normals_a=None, normals_b=None, dist_mat: Optional[torch.Tensor] = None) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:

    def one_direction(fixed: torch.Tensor, search: torch.Tensor, n_f, n_s, closest_id) -> Tuple[torch.Tensor, torch.Tensor]:
...
def is_quad(mesh: Union[Tuple[torch.Tensor, Optional[torch.Tensor]], Tuple[torch.Tensor, List[List[int]]]]) -> bool:
    if type(mesh) is torch.Tensor:
        return False
    if type(mesh[1]) is torch.Tensor:
        return False
    else:
...
def align_mesh(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], ref_vs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    vs, faces = mesh
...
def triangulate_mesh(mesh: Union[Tuple[torch.Tensor, Optional[torch.Tensor]], Tuple[torch.Tensor, List[List[int]]]]) -> Tuple[Tuple[torch.Tensor, torch.Tensor], Optional[torch.Tensor]]:

    def get_skinny(faces_) -> torch.Tensor:
        vs_faces = vs[faces_]
...
def decimate_igl(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], num_faces: int) -> Tuple[torch.Tensor, torch.Tensor]:
...
def gaussian_curvature(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]]) -> torch.Tensor:
    gc = igl.gaussian_curvature(*mesh)
    return gc


@igl_prepare(torch.float32)
def per_vertex_normals_igl(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], weighting: int = 0) -> torch.Tensor:
    normals = igl.per_vertex_normals(*mesh, weighting)
    return normals


@igl_prepare(torch.float32, torch.int64)
def remove_duplicate_vertices(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], epsilon=1e-7) -> Tuple[torch.Tensor, torch.Tensor]:
    vs, _, _, faces = igl.remove_duplicate_vertices(*mesh, epsilon)
    return vs, faces


@igl_prepare(torch.float32)
def winding_number_igl(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], query: torch.Tensor) -> torch.Tensor:
...
def principal_curvature(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
...
@igl_prepare()
def get_inside_outside(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], points: np.ndarray) -> np.ndarray:
    batch_size = 1000000
    labels = []
...
@igl_prepare()
def get_fast_inside_outside(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], points: np.ndarray) -> np.ndarray:
    batch_size = 1000000
    labels = []
    num_batch = points.shape[0] // batch_size + 1
    for i in range(points.shape[0] // batch_size + 1):
...
@igl_prepare(torch.float32, torch.int64)
def trimesh_smooth(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], lamb=0.5, iterations=10) -> Tuple[torch.Tensor, torch.Tensor]:
...
def split_by_seg(mesh: Tuple[torch.Tensor, Optional[torch.Tensor]], seg: Union[List[torch.Tensor], Tuple[torch.Tensor, ...]]) -> Union[List[torch.Tensor], Tuple[torch.Tensor, ...]]:
    ...
    for vs_ in (vs, vs_mid_faces):
        ...
        raise NotImplementedError("ChamferDistance is not available or imported.")
