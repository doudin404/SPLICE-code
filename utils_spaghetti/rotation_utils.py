...
def rot_to_quat(r: Tensor) -> Tensor:
    shape = r.shape
    r = r.view(-1, 3, 3)
...
def get_rotation_matrix(theta: float, axis: float, degree: bool = False) -> ndarray:
    if degree:
...
def get_random_rotation(batch_size: int) -> Tensor:
    r = Rotation.random(batch_size).as_matrix().astype(np.float32)
    Rotation.random()
    return torch.from_numpy(r)


def rand_bounded_rotation_matrix(cache_size: int, theta_range: float = .1) -> Callable[[int], Tensor]:
...
def transform_rotation(points: Tensor, one_axis: bool = False, max_angle: float = -1) -> Tensor:
    r = get_random_rotation(one_axis, max_angle)
    transformed = torch.einsum('nd,rd->nr', points, r)
    return transformed


def tb_to_rot(abc: Tensor) -> Tensor:
...
def rot_to_tb(rot: Tensor) -> Tensor:
...
def apply_gmm_affine(gmms: Union[Tuple[Tensor, ...], List[Tensor]], affine: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    mu, p, phi, eigen = gmms
    if affine.dim() == 2:
...
def get_reflection(reflect_axes: Tuple[bool, ...]) -> Tensor:
...
def get_tait_bryan_from_p(p: Tensor) -> Tensor:
    # p = p.squeeze(1)
    shape = p.shape
