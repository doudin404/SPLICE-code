...
import options
from model_spaghetti import models_utils, occ_gmm
from snowman_logger import create_tagged_logger
logger = create_tagged_logger('MODEL')


LI = Union[torch.Tensor, float, int]
...
def model_factory(opt: options.Options, override_model: Optional[str], device: torch.device) -> models_utils.Model:
    if override_model is None:
        return Models[opt.model_name](opt).to(device)
    return Models[override_model](opt).to(device)


def load_model(opt: options.Options, device: torch.device, suffix: str = '', override_model: Optional[str] = None) -> models_utils.Model:
...
class Logger:
    ...
    @staticmethod
    def aggregate(dictionary: dict, parent_dictionary: Optional[dict] = None) -> dict:
