...
class GmmMeshStage:
    ...
    def copy_gaussians(self, original_addresses: List[str]) -> List[str]:
        ...
        for original_address in original_addresses:
            if original_address in self.addresses_dict:
                # 获取原始的高斯对象
                original_gaussian = self.gmm[self.addresses_dict[original_address]]
                
                # 复制原始高斯对象
                gaussian_copy = original_gaussian.copy(self.render, self.view_style, is_selected=False)
                
                # 将复制体添加到 gmm 列表中并获取它的地址
                self.gmm.append(gaussian_copy)
                new_address = gaussian_copy.get_address()
                new_addresses.append(new_address)

                # 更新 addresses_dict
                self.addresses_dict[new_address] = len(self.gmm) - 1

        # 调用 make_copies 方法，将新复制体与原始对象建立共享关系
        for original_address in original_addresses:
            if original_address in self.addresses_dict:
    ...
    def make_copies(self, original_address: str, new_addresses: List[str]):
        ...
        # 如果原始对象还没有 copy_group，初始化一个新的集合
        if not hasattr(original_gaussian, 'copy_group'):
            original_gaussian.copy_group = set()

        # 将原始对象的地址添加到 copy_group 中
        original_gaussian.copy_group.add(original_address)

        # 遍历所有新复制体的地址
        for address in new_addresses:
            if address in self.addresses_dict:
                new_gaussian = self.gmm[self.addresses_dict[address]]

                # 将新复制体加入到原始对象的 copy_group 中
                original_gaussian.copy_group.add(address)
