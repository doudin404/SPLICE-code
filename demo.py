...
from ui import ui_utils,ui_controllers,inference_processing,files_utils
import abc
import numpy as np
...
def to_local(func):
    def inner(self, mouse_pos: Optional[Tuple[int, int]], *args, **kwargs):
...
class TransitionController:
    ...
    def get_delta_scaling(self, mouse_pos):
        ...
        Returns:
            A scaling vector (np.array of shape (3,)).
        """
        delta_scaling = np.ones(3)  # 初始化为全为 1 的缩放向量

        if self.edit_direction == ui_utils.EditDirection.All_Axis:
            # 原有逻辑：均匀缩放
            initial_distance = np.linalg.norm(self.origin_mouse - self.transition_origin_2d)
            ...
            # 沿当前方向计算缩放的变化
            delta = torch.einsum('d,d', vec, self.dir_2d[:, axis])  # 计算鼠标移动在该轴上的投影
            scaling_factor = 1.0 + delta * self.camera.GetDistance()  # 根据距离计算缩放因子
...
class RenderPop(StagedCanvas):
    ...
    def __init__(self, opt, min_viewport:Tuple[float, float, float, float], max_viewport: Tuple[float, float, float, float],
        ...
        self.button_minimize = ui_utils.ImageButton(
            [f'ui/ui_resources/icons-14.png', f'ui/ui_resources/icons-13.png'], iren, self, (0.015, 0.015),
...
class RenderMain(StagedCanvas):
    ...
    def toggle_edit_direction(self, direction: ui_utils.EditDirection):
        # if self.transition_controller.transition_type==ui_utils.EditType.Scaling and direction!=ui_utils.EditDirection.All_Axis:
        #     self.scale_id=self.stage.retain_first_selection()
    ...
    def temporary_transition(self, mouse_pos: Optional[Tuple[int, int]] = None, end=False,copy=False) -> bool:
        transition = self.transition_controller.get_transition(mouse_pos)
        is_change = False
        for gaussian in self.selected_gaussians:
            if end:
                if copy:
                    self.stage.copy_gaussians([gaussian.get_address()])
                is_change = gaussian.end_transition(transition) or is_change
            else:
                is_change = gaussian.temporary_transition(transition) or is_change
        return is_change

    def end_transition(self, mouse_pos: Optional[Tuple[int, int]],copy=False) -> bool:
        return self.temporary_transition(mouse_pos, True,copy=copy)
    ...
    def make_twins(self, toggled, new_addresses):
    ...
    def do_adjust(self):
        if self.model_process is not None:
            self.model_process.do_adjust(self.lock_pose,self.lock_shape,self.lock_empty,self.generate_mode)
            return True
        return False
    
    def set_pose_lock(self, is_locked: bool):
        self.lock_pose = is_locked
        print(f"lock_pose:{is_locked}")

    def set_shape_lock(self, is_locked: bool):
        self.lock_shape = is_locked
        print(f"lock_shape:{is_locked}")

    def set_empty_lock(self, is_locked: bool):
        self.lock_empty = is_locked
        print(f"lock_empty:{is_locked}")

    def set_generate_mode(self, is_generate: bool):
        self.generate_mode = is_generate
        print(f"generate_mode:{is_generate}")
    ...
    def __init__(self, cfg, viewport:Tuple[float, float, float, float], samples_root, render_window: vtk.vtkRenderWindow,
                 bg_color: ui_utils.RGB_COLOR, stroke_color: Optional[ui_utils.RGBA_COLOR], with_model: bool):
        super(RenderMain, self).__init__(cfg, viewport, render_window, bg_color, stroke_color)

        if with_model:
            self.model_process = inference_processing.InferenceProcess(cfg, self.stage.replace_mesh,
                                                                       self.stage.get_gmm,
                                                                       samples_root,
                                                                       send_gmm=self.stage.set_gmm,
                                                                       )
        else:
        ...
        self.lock_shape = True

        self.lock_empty = True
...
class MeshScroller(vtk.vtkRenderer):
    ...
    def __init__(self, opt, viewport: Tuple[float, float, float, float], render_window: vtk.vtkRenderWindow, iren,
        ...
        self.cb_stack = []
        self.button_slide = ui_utils.ImageButton([f'ui/ui_resources/icons-06.png'], iren, self, (0.05, 0.02),
...
class InteractorStyle(vtk.vtkInteractorStyleTrackballCamera):
    ...
    def on_key_press(self, obj, event):
        ...
        # 添加对 '\' 键的处理逻辑
        if key == 'backslash':
            after_gmms, included_after,_ = self.render_main.stage.get_gmm()
            included_id = included_after[0][0]
            shape_id = self.mesh_scroller.meshes.gmms[included_id].shape_id
            base_mesh = self.mesh_scroller.meshes.gmms[included_id].base_mesh
            before_gmms, included_before,_ = self.mesh_scroller.meshes.gmms[included_id].get_gmm(all_gmm=True)  # self.source_stage

            # Helper function to filter included list where the first element matches included_id
            def filter_included_by_id(included, target_id):
                return [row for row in included if row[0] == target_id]

            # Filter included_after and included_before
            filtered_included_after = filter_included_by_id(included_after, included_id)
            filtered_included_before = filter_included_by_id(included_before, included_id)

            # Find the corresponding indices from included_after and included_before
            sorted_indices_after = [row[1] for row in filtered_included_after if row[0] == included_id]
            sorted_indices_before = [row[1] for row in filtered_included_before if row[0] == included_id]

            # Create zero-filled tensors to hold the sorted gmms based on real sequence
            reordered_after_gmms = [tensor.clone().zero_() for tensor in after_gmms]
            reordered_before_gmms = [tensor.clone().zero_() for tensor in before_gmms]

            # Place each tensor in its correct position based on sorted_indices_after
            # for i, idx in enumerate(sorted_indices_after):
            #     for tensor_idx, tensor in enumerate(after_gmms):
            #         if tensor.ndim > 1:
            #             reordered_after_gmms[tensor_idx][:, :, idx, ...] = tensor[:, :, i, ...]
            #         else:
            #             reordered_after_gmms[tensor_idx][:, idx] = tensor[:, i]

            # Place each tensor in its correct position based on sorted_indices_before
            for i, idx in enumerate(sorted_indices_before):
                for tensor_idx, tensor in enumerate(before_gmms):
                    if tensor.ndim > 1:
                        reordered_before_gmms[tensor_idx][:, :, idx, ...] = tensor[:, :, i, ...]
                    else:
                        reordered_before_gmms[tensor_idx][:, idx] = tensor[:, i]

            # Convert lists back to tuple
            sorted_after_gmms = tuple(reordered_after_gmms)
            sorted_before_gmms = tuple(reordered_before_gmms)

            mesh = self.render_main.stage.mesh

            # # Define the root path where files will be saved
            root_path = r"./test_data"  # Change this to your desired path

            # Save sorted gmms and base_mesh
            # Save after gmms
            # files_utils.export_gmm(sorted_after_gmms, f"{root_path}/{shape_id}_after")

            # Save before gmms
            files_utils.export_gmm(sorted_before_gmms, f"{root_path}/{shape_id}_before")

            # # Save base mesh
            files_utils.export_mesh(base_mesh, f"{root_path}/{shape_id}_before")
            # 获取当前时间
            current_time = time.strftime("%Y%m%d%H%M%S")  # 格式化为年月日时分秒

            files_utils.export_mesh(mesh, f"{root_path}/{current_time}_after")

        
        if key == 'return' or key == 'kp_enter':
            gmms,included = self.render_main.stage.get_gmm()
            z= self.render_main.model_process.load_mid()
            mesh = self.render_main.stage.mesh
            files_utils.save_all(z,mesh,gmms,root_path="extract",name_prefix="costom")
            #self.mesh_scroller.save(self.root, gmms)
            # self.render_main.save(self.root)

        if key == 'd':
            self.render_main.do_adjust()
    ...
    def right_button_press_event(self, obj, event):
        ...
        if self.pondering and self.mouse_out_scroller()[1]:
            ...
            click_pos = self.interactor.GetEventPosition()
            if self.render_main.end_transition(click_pos,copy=True):
                self.render_main.update_mesh()
            self.edit_status = ui_utils.EditType.Pondering
        return
    ...
    def add_buttons(self, interactor):
        ...
        def toggle_pose_lock(button, __):
            self.lock_pose = button.GetRepresentation().GetState() == 0
            # 调用 self.render_main 对应的方法来处理姿态锁的更新
            self.render_main.set_pose_lock(self.lock_pose)

        def toggle_shape_lock(button, __):
            self.lock_shape = button.GetRepresentation().GetState() == 0
            # 调用 self.render_main 对应的方法来处理形状锁的更新
            self.render_main.set_shape_lock(self.lock_shape)

        def toggle_empty_lock(button, __):
            self.lock_empty = button.GetRepresentation().GetState() == 0
            # 调用 self.render_main 对应的方法来处理空锁的更新
            self.render_main.set_empty_lock(self.lock_empty)

        def toggle_generate_mode(button, __):
            self.generate_mode = button.GetRepresentation().GetState() == 0
            # 调用 self.render_main 对应的方法来处理生成模式的更新
            self.render_main.set_generate_mode(self.generate_mode)
        ...
        button_pencil = ui_utils.ImageButton([f"ui/ui_resources/icons-03.png", f"ui/ui_resources/icons-04.png"],
                                             interactor, self.render_main, (.08, .08), (0.01, 0.1), toggle_select_mode, full_size=(1., .85))
        button_reset = ui_utils.ImageButton([f"ui/ui_resources/icons-05.png"], interactor, self.render_main,
                                            (.08, .08), (0.01, 0.98), reset, full_size=(1., .85))
        

        # Assuming interactor and render_main are already defined.
        button_spacing = 0.08  # Spacing between buttons
        button_size = (0.08, 0.08)  # Button size (width, height)
        initial_position_x = 0.9  # Starting position for the first button on the x-axis (near the right edge)
        position_y = 0.98  # Position for the buttons on the y-axis (near the top edge)
        button_pose = ui_utils.ImageButton([
            f"ui/ui_resources/icons-03.png", f"ui/ui_resources/icons-04.png"
        ], interactor, self.render_main, button_size, (initial_position_x -3*button_spacing, position_y), on_click=toggle_pose_lock, full_size=(1., 0.85))

        button_shape = ui_utils.ImageButton([
            f"ui/ui_resources/icons-03.png", f"ui/ui_resources/icons-04.png"
        ], interactor, self.render_main, button_size, (initial_position_x -2*button_spacing, position_y), on_click=toggle_shape_lock, full_size=(1., 0.85))

        button_add = ui_utils.ImageButton([
            f"ui/ui_resources/icons-03.png", f"ui/ui_resources/icons-04.png"
        ], interactor, self.render_main, button_size, (initial_position_x - 1 * button_spacing, position_y), on_click=toggle_empty_lock, full_size=(1., 0.85))

        button_adj = ui_utils.ImageButton([
            f"ui/ui_resources/icons-03.png", f"ui/ui_resources/icons-04.png"
        ], interactor, self.render_main, button_size, (initial_position_x - 0 * button_spacing, position_y), on_click=toggle_generate_mode, full_size=(1., 0.85))

        return button_pencil, button_reset,button_pose,button_shape,button_add,button_adj
    ...
    def __init__(self, cfg, mesh_scroller: MeshScroller, render_pop: RenderPop, render_main: RenderMain, interactor):
        ...
        self.root = f"ui/ui_export/full_model/"
    
def run(cfg, with_model: bool = True):
    #opt = options.Options(tag=model_tag).load()
    samples_root = f'extract/'
    ...
    renderer_main = RenderMain(cfg, (0., .15, 1., 1.), samples_root, render_window, ui_utils.bg_source_color,
                                          list(ui_utils.bg_target_color) + [200], with_model)
    mesh_scroller = MeshScroller(cfg, (0., 0., 1., .15), render_window, interactor, ui_utils.bg_target_color,
                                 samples_root)
    renderer_pop = RenderPop(cfg, min_viewport, max_viewport, render_window, interactor, ui_utils.bg_stage_color,
                             list(ui_utils.bg_source_color) + [200], renderer_main.update_gmm)
    render_window.Render()
    interactor.Initialize()
    style = InteractorStyle(cfg, mesh_scroller, renderer_pop, renderer_main, interactor)
...
@hydra.main(config_path="config", config_name="config_ui", version_base="1.2")
