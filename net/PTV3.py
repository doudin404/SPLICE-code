...
class PointTransformerV3Encoder(nn.Module):
    def __init__(
        ...
        # 全局池化，将空间维度压缩为一个特征向量
        #self.global_pool = nn.AdaptiveAvgPool1d(0)
        self.output=nn.Linear(enc_channels[-1], out_channels)

    def forward(self, data_dict):
        # 初始化 Point 对象并进行序列化和稀疏化
        point = Point(data_dict)
        ...
        # 将稀疏特征提取为一个全局特征向量
        global_feature = point.feat

        global_feature=self.output(global_feature)

        return global_feature


class PartsToPointTransformerV3Encoder(nn.Module):
    def __init__(
        self, 
        in_channels=6,
        order=("hilbert", "hilbert-trans"),
        ...
        super(PartsToPointTransformerV3Encoder, self).__init__()

        # 初始化 PointTransformerV3Encoder，传递所有需要的参数
        self.point_transformer_encoder = PointTransformerV3Encoder(
            in_channels=in_channels,
            order=order,
            stride=stride,
...
class PartsToPointTransformerV3Encoder(nn.Module):
    def __init__(
        ...
        pdnorm_conditions=("ScanNet", "S3DIS", "Structured3D"),
        out_channels=512,
        #shape_spatial_shape=[64,64,64]
    ):
        super(PartsToPointTransformerV3Encoder, self).__init__()
        #self.shape_spatial_shape=self.shape_spatial_shape
    ...
    def forward(self, parts_voxels, parts_labels, remove_zero_labels=True):
        ...
        for batch_idx, (batch_voxels, labels_batch) in enumerate(zip(parts_voxels, parts_labels)):
            for coords, labels in zip(batch_voxels, labels_batch):
                ...
                all_coords.append(coords)  # 将当前 part 的坐标加入总列表
                all_labels.append(labels.float().unsqueeze(1))  # 将标签加入总列表

                # 生成与该 part 的坐标相同数量的批次索引
                batch_indices = torch.full(
                    (coords.size(0),),  # 坐标的数量决定批次索引的数量
                    current_batch_index,
                    dtype=torch.int64,
        ...
        # 设置所有有效体素的features为1
        features = torch.ones((labels.size(0), 1), dtype=torch.float32, device=labels.device)
        
        # 将标签信息与 features 合并
        features = torch.cat([features, labels], dim=1)

        # 在将坐标放入特征之前，将坐标与其他特征合并
        features_with_coords = torch.cat([coords/64, features], dim=1)

        # 构造 data_dict 来作为 PointTransformerV3Encoder 的输入
        data_dict = {
            "feat": features_with_coords,  # 现在特征包含了坐标
            "coord": coords,  # 体素坐标
            "batch": batch_indices,  # 每个点的批次索引
            "grid_size": torch.tensor([1.0], device=coords.device)  # 假定一个单位的 grid size
        }

        # 使用 PointTransformerV3Encoder 对点云进行编码
        encoded_features = self.point_transformer_encoder(data_dict)

        # 按照 batch_sizes 分割编码后的特征，恢复成原始的格式
