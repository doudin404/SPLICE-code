...
if __name__ == '__main__':
    ...
    # 数据模块
    files = 'data/chair_feats_4'  # 替换为实际路径
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="best_overfit_4",                # 最佳模型文件名前缀
        save_top_k=1,                     # 只保留最优模型
        monitor="val_loss",             # 监控指标，可根据需改
        mode="min",                     # 指标越小越好
        save_last=True                    # 额外保存最后一次训练的模型
    )
    checkpoint_callback.CHECKPOINT_NAME_LAST = f"overfit_4"########################
    ...
    last_ckpt = "checkpoints_splice_diff/overfit_4.ckpt"
    if last_ckpt:
